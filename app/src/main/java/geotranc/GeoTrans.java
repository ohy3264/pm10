...
public class GeoTrans {
	...
	static {
		...
		src_m[GEO] = m_arMajor[GEO]
				* mlfn(e0fn(m_Es[GEO]), e1fn(m_Es[GEO]), e2fn(m_Es[GEO]),
						e3fn(m_Es[GEO]), m_arLatCenter[GEO]);
		dst_m[GEO] = m_arMajor[GEO]
				* mlfn(e0fn(m_Es[GEO]), e1fn(m_Es[GEO]), e2fn(m_Es[GEO]),
						e3fn(m_Es[GEO]), m_arLatCenter[GEO]);
		src_m[KATEC] = m_arMajor[KATEC]
				* mlfn(e0fn(m_Es[KATEC]), e1fn(m_Es[KATEC]), e2fn(m_Es[KATEC]),
						e3fn(m_Es[KATEC]), m_arLatCenter[KATEC]);
		dst_m[KATEC] = m_arMajor[KATEC]
				* mlfn(e0fn(m_Es[KATEC]), e1fn(m_Es[KATEC]), e2fn(m_Es[KATEC]),
						e3fn(m_Es[KATEC]), m_arLatCenter[KATEC]);
		src_m[TM] = m_arMajor[TM]
				* mlfn(e0fn(m_Es[TM]), e1fn(m_Es[TM]), e2fn(m_Es[TM]),
						e3fn(m_Es[TM]), m_arLatCenter[TM]);
		dst_m[TM] = m_arMajor[TM]
				* mlfn(e0fn(m_Es[TM]), e1fn(m_Es[TM]), e2fn(m_Es[TM]),
						e3fn(m_Es[TM]), m_arLatCenter[TM]);
		src_m[GRS80] = m_arMajor[GRS80]
				* mlfn(e0fn(m_Es[GRS80]), e1fn(m_Es[GRS80]), e2fn(m_Es[GRS80]),
						e3fn(m_Es[GRS80]), m_arLatCenter[GRS80]);
		dst_m[GRS80] = m_arMajor[GRS80]
				* mlfn(e0fn(m_Es[GRS80]), e1fn(m_Es[GRS80]), e2fn(m_Es[GRS80]),
						e3fn(m_Es[GRS80]), m_arLatCenter[GRS80]);
		src_m[UTMK] = m_arMajor[UTMK]
				* mlfn(e0fn(m_Es[UTMK]), e1fn(m_Es[UTMK]), e2fn(m_Es[UTMK]),
						e3fn(m_Es[UTMK]), m_arLatCenter[UTMK]);
		dst_m[UTMK] = m_arMajor[UTMK]
				* mlfn(e0fn(m_Es[UTMK]), e1fn(m_Es[UTMK]), e2fn(m_Es[UTMK]),
						e3fn(m_Es[UTMK]), m_arLatCenter[UTMK]);
	}
	...
	private static double mlfn(double e0, double e1, double e2, double e3,
			double phi) {
	...
	public static void geo2tm(int dsttype, GeoPoint in_pt, GeoPoint out_pt) {
		...
		if (m_Ind[dsttype] != 0) {
			...
			double b = 0;
			x = 0.5 * m_arMajor[dsttype] * m_arScaleFactor[dsttype]
					* Math.log((1.0 + b) / (1.0 - b));
			double con = Math.acos(cos_phi * Math.cos(delta_lon)
					/ Math.sqrt(1.0 - b * b));

			if (in_pt.y < 0) {
				con = con * -1;
				y = m_arMajor[dsttype] * m_arScaleFactor[dsttype]
						* (con - m_arLatCenter[dsttype]);
			}
		}
		...
		double ml = m_arMajor[dsttype]
				* mlfn(e0fn(m_Es[dsttype]), e1fn(m_Es[dsttype]),
						e2fn(m_Es[dsttype]), e3fn(m_Es[dsttype]), in_pt.y);

		out_pt.x = m_arScaleFactor[dsttype]
				* n
				* al
				* (1.0 + als
						/ 6.0
						* (1.0 - t + c + als
								/ 20.0
								* (5.0 - 18.0 * t + t * t + 72.0 * c - 58.0 * m_Esp[dsttype])))
				...
				* (ml - dst_m[dsttype] + n
						...
						* (als * (0.5 + als
								...
								* (5.0 - t + 9.0 * c + 4.0 * c * c + als
										/ 30.0
										* (61.0 - 58.0 * t + t * t + 600.0 * c - 330.0 * m_Esp[dsttype])))))
	...
	public static void tm2geo(int srctype, GeoPoint in_pt, GeoPoint out_pt) {
		...
		if (m_Ind[srctype] != 0) {
			double f = Math.exp(in_pt.x
					/ (m_arMajor[srctype] * m_arScaleFactor[srctype]));
			double g = 0.5 * (f - 1.0 / f);
			double temp = m_arLatCenter[srctype] + tmpPt.y
					/ (m_arMajor[srctype] * m_arScaleFactor[srctype]);
		...
		double con = (src_m[srctype] + tmpPt.y / m_arScaleFactor[srctype])
				/ m_arMajor[srctype];
		double phi = con;
		...
		while (true) {
			double delta_Phi = ((con + e1fn(m_Es[srctype])
					* Math.sin(2.0 * phi) - e2fn(m_Es[srctype])
					* Math.sin(4.0 * phi) + e3fn(m_Es[srctype])
					* Math.sin(6.0 * phi)) / e0fn(m_Es[srctype]))
					- phi;
			phi = phi + delta_Phi;
			...
			out_pt.x = m_arLonCenter[srctype]
					+ (d
							* (1.0 - ds
									...
									* (1.0 + 2.0 * t + c - ds
											/ 20.0
											* (5.0 - 2.0 * c + 28.0 * t - 3.0
													* cs + 8.0 * m_Esp[srctype] + 24.0 * ts))) / cos_phi);
		} else {
	...
	public static double getDistancebyGeo(GeoPoint pt1, GeoPoint pt2) {
		...
		double a = Math.pow(Math.sin(latitude / 2.0), 2) + Math.cos(lat1)
				* Math.cos(lat2) * Math.pow(Math.sin(longitude / 2.0), 2);
	...
	private static void transform(int srctype, int dsttype, GeoPoint point) {
		...
		if ((srctype != 0 && srctype != GRS80 && srctype != UTMK)
				|| (dsttype != 0 && dsttype != GRS80 && dsttype != UTMK)) {
	...
	private static boolean geodetic_to_geocentric(int type, GeoPoint p) {
		 ...
		 * should be wrapped by Math.cos() and Math.sin(). NFW for PROJ.4,
		 * Sep/2001.
		 */
	...
	private static void geocentric_to_geodetic(int type, GeoPoint p) {
		...
		double Sin_B0; /*
						 * Math.sin(B0), B0 is estimate of Bowring aux
						 * doubleiable
						 */
